    ...
    private func infinityThumbImage() -> UIImage? {
        ...
        return renderer.image { ctx in
            ...
            if let symImage = UIImage(systemName: "infinity", withConfiguration: config)?.withTintColor(.black, renderingMode: .alwaysOriginal) {
                ...
                let y = (size.height - imgSize.height) / 2 - 1
    ...
    var body: some View {
        ZStack {
            ...
            VStack {
                ...
                HStack(spacing: 40) {
                    Button {
                        print("Sun button tapped, setting dim duration to 360 seconds")
                    ...
                    Button {
                        dimMode = .duration(4) // 4 seconds
                        dimOverlayOpacity = 0
                        withAnimation(.linear(duration: 4)) { // 4 seconds
                            dimOverlayOpacity = 1
                        }
                    } label: {
...
struct ContentView: View {
    ...
    @State private var durationMinutes: Double = 0.0
